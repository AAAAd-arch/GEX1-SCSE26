...
def find_flight(flights, flight_number):
    flight_number = flight_number.strip().lower()

    for key in flights:
        if key.lower() == flight_number:
            return key
...
def passenger_exists(passengers, passenger_name):
    ...
    for passenger in passengers:
        if passenger.lower() == passenger_name:
            return True
...
def check_in_passenger(
    ...
    for destination in restricted_destinations:
        if destination.lower() == flight["destination"].lower():
    ...
    flight["passengers"].append(passenger_name.title())
    return "OK"
...
def change_gate(
    ...
    new_gate = new_gate.strip().lower()
    gate_key = None

    for gate in allowed_gates:
        if gate.lower() ==new_gate:
            gate_key = gate
            break
    ...
    flights[flight_key]["gate"] = gate_key
    return "OK"
